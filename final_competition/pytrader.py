...
class pytrader:
    ...
    def contract(self, code):
        ...
        if(sum(item['size'] for item in self.orders) != 0):
            ...
            self.position['is_long'] = True if self.position['size'] > 0 else False
            self.position['is_short'] = True if self.position['size'] < 0 else False

            if self.position['size'] > 0:
                self.position['pnl'] = round(((float(msg_queue['bidask'][-1]['bid_price'][0])*0.99998 - self.trades[-1]
                                               ['entry_price'])*200*self.position['size']) - 100 - self.trades[-1]['tax'])
                ...
                self.position['pnl'] = round(((self.trades[-1]['entry_price'] - float(msg_queue['bidask'][-1]
                                                                                      ['ask_price'][0])*1.00002)*200*-self.position['size']) - 100 - self.trades[-1]['tax'])
                self.position['pl_pct'] = (
    ...
    def list_positions(self):
        ...
        if(self.simulation == False):
            ...
            print('use self.position instead')
            return
